...
namespace System.Windows.Controls.Primitives
{
    ...
    [TemplatePart(Name = CalendarExItem.ElementRoot, Type = typeof(FrameworkElement))]
    [TemplatePart(Name = CalendarExItem.ElementHeaderButton, Type = typeof(Button))]
    [TemplatePart(Name = CalendarExItem.ElementPreviousButton, Type = typeof(Button))]
    [TemplatePart(Name = CalendarExItem.ElementNextButton, Type = typeof(Button))]
    [TemplatePart(Name = CalendarExItem.ElementDayTitleTemplate, Type = typeof(DataTemplate))]
    [TemplatePart(Name = CalendarExItem.ElementMonthView, Type = typeof(Grid))]
    [TemplatePart(Name = CalendarExItem.ElementYearView, Type = typeof(Grid))]
    [TemplatePart(Name = CalendarExItem.ElementDisabledVisual, Type = typeof(FrameworkElement))]
    [TemplateVisualState(Name = VisualStates.StateNormal, GroupName = VisualStates.GroupCommon)]
    [TemplateVisualState(Name = VisualStates.StateDisabled, GroupName = VisualStates.GroupCommon)]
    public sealed partial class CalendarExItem : Control
    {
        /// <summary>
        ...
        /// </summary>
        internal CalendarEx Owner { get; set; }

        /// <summary>
        /// Inherited code: Requires comment.
        /// </summary>
        private CalendarExButton _lastCalendarButton;

        /// <summary>
        /// Inherited code: Requires comment.
        /// </summary>
        private CalendarExDayButton _lastCalendarDayButton;

        /// <summary>
        /// Gets or sets Inherited code: Requires comment.
        /// </summary>
        internal CalendarExDayButton CurrentButton { get; set; }
        ...
        /// class.
        /// </summary>
        public CalendarExItem()
        {
            DefaultStyleKey = typeof(CalendarExItem);
        }
        ...
        public override void OnApplyTemplate()
        {
            ...
            if (MonthView != null && YearView != null)
            {
                if (Owner != null)
                {
                    ...
                    if (Owner.DisplayMode == CalendarExMode.Year)
                    {
                        UpdateYearMode();
                    }
                    else if (Owner.DisplayMode == CalendarExMode.Decade)
                    {
                        UpdateDecadeMode();
                    }

                    if (Owner.DisplayMode == CalendarExMode.Month)
                    {
                        UpdateMonthMode();
        ...
        private void PopulateGrids()
        {
            if (MonthView != null)
            {
                for (int i = 0; i < CalendarEx.RowsPerMonth; i++)
                {
                ...
                for (int i = 1; i < CalendarEx.RowsPerMonth; i++)
                {
                    for (int j = 0; j < CalendarEx.ColumnsPerMonth; j++)
                    {
                        CalendarExDayButton cell = new CalendarExDayButton();
            ...
            if (YearView != null)
            {
                CalendarExButton month;
                int count = 0;
                for (int i = 0; i < CalendarEx.RowsPerYear; i++)
                {
                    for (int j = 0; j < CalendarEx.ColumnsPerYear; j++)
                    {
                        month = new CalendarExButton();
        ...
        {
            for (int childIndex = 0; childIndex < CalendarEx.ColumnsPerMonth; childIndex++)
            {
        ...
        private void SetButtonState(CalendarExDayButton childButton, DateTime dateToAdd)
        {
            if (Owner != null)
            {
        ...
        private void SetCalendarDayButtons(DateTime firstDayOfMonth)
        {
            ...
            int count = CalendarEx.RowsPerMonth * CalendarEx.ColumnsPerMonth;

            for (int childIndex = CalendarEx.ColumnsPerMonth; childIndex < count; childIndex++)
            {
                CalendarExDayButton childButton = MonthView.Children[childIndex] as CalendarExDayButton;
                ...
                else
                {
                    ...
                    {
                        childButton = MonthView.Children[i] as CalendarExDayButton;
                        Debug.Assert(childButton != null, "childButton should not be null!");
            ...
            if (Owner != null && Owner.HoverStart.HasValue && Owner.HoverEndInternal.HasValue)
            {
                if (!Owner.HoverEndIndex.HasValue)
                {
                    ...
                    {
                        Owner.HoverEndIndex = CalendarEx.ColumnsPerMonth * CalendarEx.RowsPerMonth - 1;
                    }
                    else
                    {
                        Owner.HoverEndIndex = CalendarEx.ColumnsPerMonth;
                    }
                }
                ...
                if (!Owner.HoverStartIndex.HasValue)
                {
                    if (DateTimeHelper.CompareDays(Owner.HoverEndInternal.Value, Owner.HoverStart.Value) > 0)
                    {
                        Owner.HoverStartIndex = CalendarEx.ColumnsPerMonth;
                    }
                    else
                    {
                        Owner.HoverStartIndex = CalendarEx.ColumnsPerMonth * CalendarEx.RowsPerMonth - 1;
                    }
                }
            }
        }
        ...
        private void SetMonthButtonsForYearMode()
        {
            ...
            {
                CalendarExButton childButton = child as CalendarExButton;
                Debug.Assert(childButton != null, "childButton should not be null!");
        ...
        internal void UpdateYearViewSelection(CalendarExButton calendarButton)
        {
        ...
        internal void Cell_MouseEnter(object sender, MouseEventArgs e)
        {
            if (Owner != null)
            {
                CalendarExDayButton b = sender as CalendarExDayButton;
                if (_isMouseLeftButtonDown && b != null && b.IsEnabled && !b.IsBlackout)
                {
                    ...
                    {
                        case CalendarExSelectionMode.SingleDate:
                            {
                        ...
                        case CalendarExSelectionMode.SingleRange:
                        case CalendarExSelectionMode.MultipleRange:
                            {
            ...
            if (Owner != null)
            {
                ...
                if (b != null)
                {
                    if (b.IsEnabled && !b.IsBlackout)
                    {
                        ...
                        switch (Owner.SelectionMode)
                        {
                            case CalendarExSelectionMode.None:
                                {
                                    return;
                                }
                            case CalendarExSelectionMode.SingleDate:
        ...
        private void AddSelection(CalendarExDayButton b)
        {
            if (Owner != null)
            {
                Owner.HoverEndIndex = b.Index;
        ...
        internal void Cell_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            if (Owner != null)
            {
                ...
                {
                    if (Owner.SelectionMode == CalendarExSelectionMode.None || Owner.SelectionMode == CalendarExSelectionMode.SingleDate)
                    {
                        Owner.OnDayClick((DateTime) b.DataContext);
                        return;
                    }
        ...
        private void Cell_Click(object sender, RoutedEventArgs e)
        {
            if (Owner != null)
            {
                ...
                if (ctrl && Owner.SelectionMode == CalendarExSelectionMode.MultipleRange)
                {
                    CalendarExDayButton b = sender as CalendarExDayButton;
            ...
            if (Owner != null)
            {
                ...
                if (b.IsEnabled)
                {
                    if (Owner.DisplayMode == CalendarExMode.Month)
                    {
                        ...
                        Owner.DisplayMode = CalendarExMode.Year;
                    }
                    else
                    {
                        Debug.Assert(Owner.DisplayMode == CalendarExMode.Year, "The Owner Calendar's DisplayMode should be Year!");
                        ...
                        Owner.DisplayMode = CalendarExMode.Decade;
                    }
                }
            }
        }
        ...
        {
            CalendarExButton b = sender as CalendarExButton;
            Debug.Assert(b != null, "The sender should be a non-null CalendarDayButton!");
        ...
        internal void Month_CalendarButtonMouseUp(object sender, MouseButtonEventArgs e)
        {
            ...
            if (Owner != null)
            {
                DateTime newmonth = (DateTime)((CalendarExButton)sender).DataContext;

                if (Owner.DisplayMode == CalendarExMode.Year)
                {
                    Owner.DisplayDate = newmonth;
                    Owner.DisplayMode = CalendarExMode.Month;
                }
                else
                {
                    Debug.Assert(Owner.DisplayMode == CalendarExMode.Decade, "The owning Calendar should be in decade mode!");
